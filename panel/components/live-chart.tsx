...
function readAppearance(): ChartAppearance {
  ...
  // cannot drift when the palette is retuned. The token NAMES come from
  // lib/tradingview.ts#TV_PALETTE_ROLES, which the Drivers card's Mini Chart
  // embeds read too — one palette, two widget generations.
  const style = getComputedStyle(root);
  const token = (name: string, fallback: string) =>
    style.getPropertyValue(name).trim() || fallback;
  return {
    theme: dark ? "dark" : "light",
    backgroundColor: token(TV_PALETTE_ROLES.background, dark ? "#0a0b0a" : "#f6f6f3"),
    gridColor: token(TV_PALETTE_ROLES.grid, dark ? "#333330" : "#dedad2"),
  };
}
